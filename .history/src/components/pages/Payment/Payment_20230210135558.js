...
const Payment = () => {
  ...
  const fetchData = async () => {
    try{
      const { data } = await axios.get("http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5002/api/categorys/getcategorybyadmin/noida")
      setData(data)
    }catch(err){
      console.log(err)
    }
  ...
  const deleteHandler = async (id) => {
    try{
      const { data } = await axios.delete(`http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5002/api/categorys/deletecategorybyadmin/${id}`)
...
function MyVerticallyCenteredModal(props) {
  ...
  const AddData = async (e) => {
    e.preventDefault()
    try{
      const { data  } = await axios.post("http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5002/api/categorys/shoot/" , {name , desc , photoActual , photoExcepted , videoActual , videoExcepted , dronActual , dronExcepted })
    }catch(err){
      console.log(err)
    }
  }
            ...
            {data?.map((i, index) => (
              <tr key={index}>
                ...
                <td>
                  {" "}
                  ₹{i.photoActual}
                  <p style={{ textDecoration: "line-through" }}> ₹{i.photoExcepted} </p>
                </td>
                <td>
                  ₹ {i.videoActual}
                  <p style={{ textDecoration: "line-through" }}> ₹{i.videoExcepted} </p>{" "}
                </td>
                <td>
                  {" "}
                  ₹{i.dronActual}
                  <p style={{ textDecoration: "line-through" }}> ₹{i.dronExcepted} </p>{" "}
                </td>
                <td>
