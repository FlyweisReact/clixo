  ...
  const filterData = useCallback(async (url) => {
    try {
      const { data } = await axios.get(
        `https://5mkj1ib4ve.execute-api.ap-south-1.amazonaws.com/dev/api/getMonthwise/${url ? url : 'today'}`
      );
      setData(data);
    } catch (err) {
      console.log(err);
...
const fetchData = async () =>{
  try{
    const { data } = await axios.get("https://5mkj1ib4ve.execute-api.ap-south-1.amazonaws.com/dev/api/common/all")
    setCount(data.result.len)
  }catch(E) { 
    console.log(E)
  }
}
  ...
  const card = [
    ...
    },
    {
      ...
      number: data?.data?.booking ?   data?.data?.booking : 0,
      img: `${img4}`,
      link: "/Notice/Labour",
      bg: "#ffb64d",
    },
    {
      title: "Total Hero's",
      number: data?.data?.hero  ? data?.data?.hero : 0,
...

    {
      ...
      number: data?.data?.user ? data?.data?.user : 0,
    ...
    <>
      ...
      <div className="dropdown">
        ...
        <div className="dropdown-content">
          <p onClick={() => filterData("weekly")}> Weakly</p>
          <p onClick={() => filterData("monthly")}> Monthly</p>
          <p onClick={() => filterData("monthly")}> Yearly</p>
        </div>
      </div>
