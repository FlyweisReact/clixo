...
const GenerateId = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const { data } = await axios.get(
        "https://5mkj1ib4ve.execute-api.ap-south-1.amazonaws.com/dev/api/heros/getherorole4"
      );
      setData(data.HERO.reverse());
    } catch (err) {
      console.log(err);
    }
  }, []);
  ...
  const fetchSignlerHero = useCallback(async () => {
    try {
      const { data } = await axios.get(
        `https://5mkj1ib4ve.execute-api.ap-south-1.amazonaws.com/dev/api/heros/getherobyid/${id}`
      );
      setEach(data);
    } catch (err) {}
  }, [id]);
    ...
    const AddHero = async (e) => {
      ...
      try {
        const data = await axios.post(
          "https://5mkj1ib4ve.execute-api.ap-south-1.amazonaws.com/dev/api/heros/addherobyadmin",
          formD
        );
        console.log(data);
        props.onHide();
  ...
  function StatusModal(props) {
    ...
    const disableHero = async (status) => {
      try {
        const data = await axios.put(
          `https://5mkj1ib4ve.execute-api.ap-south-1.amazonaws.com/dev/api/heros/changeStatusbyid_by_dmin/${id}`,
          {
            status: status,
          }
        );
    ...
    return (
      ...
      >
        <Modal.Body>
          <Container>
          <div >
              <img src={warning} alt="" style={{ width: "100px", display: "block", margin: "auto" }} />
            </div>
          {statusEdit ? 
    ...
    const AddWalletHandler = async (e) => {
      ...
      try {
        if (dropDown === "removebalance") {
          const data = await axios.post(
            `https://5mkj1ib4ve.execute-api.ap-south-1.amazonaws.com/dev/api/wallet/addandremoveMoneyinadminofhero/${id}`,
            {
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(
        `https://5mkj1ib4ve.execute-api.ap-south-1.amazonaws.com/dev/api/heros/deleteheroByIdinadmin/${id}`
      );
                ...
                <tr key={index}>
                  ...
                  <td>
                    ...
                    <Button
                      ...
                      onClick={() => navigate(`/heroBooking/${i._id}`)}
                    >
                      Project History
                    </Button>{" "}
                  </td>
